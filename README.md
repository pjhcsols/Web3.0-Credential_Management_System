...
이 프로젝트에서는 OAuth 2.0 프로토콜을 기반으로 소셜 로그인 기능을 구현하였습니다. 특히, 카카오 API를 활용하여 서버에서 카카오 엑세스 토큰을 발급 받아 사용자의 소셜 계정 정보를 안전하게 가져오고, 이를 통해 시스템에 손쉽게 회원가입과 로그인을 수행할 수 있습니다.
...
본 프로젝트에서는 기본적인 로그인 절차 외에도 추가적인 보안 강화를 위해 2차 인증 기능을 포함시켰습니다. 특히, 생체 인증을 활용한 2차 인증 절차를 통해 사용자는 보다 안전하게 시스템에 접근할 수 있습니다.

Android 기기에서는 BiometricPrompt API를, iOS 기기에서는 LocalAuthentication Framework를 사용하여 지문, 얼굴 인식과 같은 생체 정보를 이용한 인증을 수행합니다. 이 생체 인증 절차는 전자 지갑 생성을 위한 필수 단계로, 사용자의 신원을 한층 더 강력하게 보호합니다.

2차 인증이 성공적으로 완료된 후에만 사용자는 Web3 기반의 전자 지갑을 생성할 수 있으며, 이 지갑은 블록체인 상에서의 다양한 거래와 상호작용을 가능하게 합니다. 이를 통해 시스템은 사용자의 자산과 개인 정보를 더욱 철저히 보호하며, 높은 수준의 보안 요구 사항을 충족시킵니다.

생체 인증 절차는 사용자의 편의성을 해치지 않으면서도 보안을 강화하는 중요한 역할을 하며, 특히 금융 서비스나 중요한 데이터에 접근할 때 필수적인 추가 보안 계층을 제공합니다.
...
### 1.4. 블록체인의 블록과 S3 메타데이터를 통한 인증 수행을 위한 Verifiable Credential 관리
