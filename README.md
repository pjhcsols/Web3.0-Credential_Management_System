...
### 1. RSA 기반 인증서 관리
- **개인 키 (Private Key)**: 개인별로 UUID를 사용하여 생성된 개인 키로 인증서의 PDF 메타데이터와 블록 데이터를 암호화합니다.
- **공개 키 (Public Key)**: PDF 파일의 SHA-256 해시값을 공개 키로 저장하며, S3에 저장된 PDF 파일과 일치하지 않으면 데이터 복호화를 제한합니다. 인증 시간만이 암호화되지 않은 형태로 표시되어 블록체인 상에서 인증 시간을 확인할 수 있습니다.

### 2. OAuth2.0 및 Web3 기반 통합 인증서 관리
- **OAuth2.0과 생체 인증**: OAuth2.0 프로토콜을 사용하여 사용자로부터 액세스 토큰을 발급받고 생체 인증으로 추가 사용자 검증을 수행하여 지갑 접근에 대한 신뢰성을 높입니다.
- **Web3 기반 지갑 관리**: 사용자 정보는 서버에 저장하지 않으며, 암호화된 PDF 메타데이터와 블록체인 데이터는 사용자가 디코딩하여 수정 및 관리할 수 있습니다. 서버는 지갑 데이터 접근 권한을 부여하기 전에 메타데이터와 블록체인 정보가 일치하는지 검증합니다.

### 3. 체인코드 및 PDF 메타데이터 기반의 인증서 관리
- **체인코드를 통한 블록 접근**: 사용자는 인증서 발급 또는 진위 확인을 요청할 수 있으며, PDF의 해시값을 RSA 공개 키로 암호화하여 블록체인에 저장하고 필요 시 블록에서 해당 데이터를 복호화하여 접근할 수 있습니다.
- **S3 접근 및 진위 확인 API 연동**: 복호화된 키값을 기반으로 외부 API를 호출하여 인증서가 유효한지 확인한 뒤, 확인된 인증서를 사용자의 전자 지갑에 발급하여 저장합니다.

### 4. 환경 변수 & uuid를 통한 보안 키 관리
- 민감한 보안 키는 HSM 대신 환경 변수를 통해 관리할 수 있습니다. 이를 통해 외부 주입 방식으로 보안 키를 안전하게 관리하며, 특히 RSA 키와 같은 중요한 데이터를 암호화하여 보호할 수 있습니다. ConfigurationPropertiesScan 어노테이션을 사용하여 환경 변수를 외부에서 자동으로 주입하고, CI/CD 과정에서 sh 스크립트를 통해 암호화 값을 개발자가 관리하고 사용자 별 uuid 값을 통해 보안성을 강화하였습니다.
...
- **private_key**: 메타데이터 디코딩용 개인 키, RSA 암호화 시 사용.
- **public_key**: PDF 파일의 해시값을 저장하여 인증서 진본성 확인.
...
## 환경 변수 설정 예시
<img width="760" alt="image" src="https://github.com/user-attachments/assets/006528e2-24c2-41d4-81d9-a10a7b28c1b7">


- `RSA_KEY`: RSA 키 값은 환경 변수로 설정하여 보안성을 강화합니다.
- **CI/CD 설정**: CI/CD 환경에서는 자동으로 암호 값을 주입받아 시스템의 보안을 더욱 강화합니다.
